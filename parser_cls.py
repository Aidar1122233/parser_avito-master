...
CITY = config.get("search", "city")
CATEGORY = config.get("search", "category")
PRICE_FROM = config.getint("search", "price_from")
PRICE_TO = config.getint("search", "price_to")
BRAND = config.get("search", "brand")
MODEL = config.get("search", "model")
...
class AvitoParse:
    ...
    def __init__(self, config_file="settings.ini"):
        ...
        # Настройки поиска
        self.city = self.config.get("search", "city", fallback="all")
        self.category = self.config.get("search", "category", fallback="avtomobili")
        self.price_from = self.config.getint("search", "price_from", fallback=1000000)
        self.price_to = self.config.getint("search", "price_to", fallback=3000000)
        self.brand = self.config.get("search", "brand", fallback="toyota").lower()
        self.model = self.config.get("search", "model", fallback="camry").lower()

        self.base_url = f"https://www.avito.ru/{self.city}/{self.category}"
        self.headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"
        }

    def get_listings(self):
        if not hasattr(self, 'proxy'):  # Проверка, что атрибут proxy существует
            logger.error("Атрибут proxy не инициализирован!")
            return []

        proxies = None
        if self.proxy:  # Проверка, что proxy не None
            logger.debug(f"Используется прокси: {self.proxy}")
            proxies = {
                "http": f"http://{self.proxy}",
                "https": f"http://{self.proxy}"
            }
        else:
            logger.debug("Прокси не используется.")

        response = requests.get(self.base_url, headers=self.headers, proxies=proxies)
    def get_listings(self):
        try:
            logger.debug(f"Запрос к URL: {self.base_url}")
            ...
            for item in soup.find_all("div", class_="iva-item-content-rejJg"):
                title_tag = item.find("h3")
                price_tag = item.find("span", class_="price-text")
                link_tag = item.find("a", class_="link-link-MbQDP")

                if title_tag and price_tag and link_tag:
                    title = title_tag.text.strip()
                    price = price_tag.text.replace("\xa0", "").strip()
